...
from utils.utils import check_inputs, create_dataloader, load_args_dic


def main(
        type_dataset: str = 'image',
        ...
        url_folder_experiment: str = './models_data/first_try/',
    ...
    Inputs:
    ------
        type_dataset (str): type of dataset on which to train the model.
            Can be "image" or "hdf5".
        ...
        list_keys_hdf5_original (List[str]) : when type type_dataset
            is 'hdf5', this list lists the keys of the hdf5 files
            from which the dataset will be created.

        key_data (str): when type type_dataset
            is 'hdf5', this is the field that corresponds to the
            data that will be generated.

        key_other (str): when type type_dataset
            is 'hdf5', this is field that is easy to load and for which
            its first dimension corresponds to the number of examples in
            the hdf5 file. This speeds up the creation of datasets.
    ...
    data_val = create_dataloader(
        args_dic['data_val_dir'],
        data_type=type_dataset,
        num_channels=args_dic['num_input_channels'],
        image_size=args_dic['image_size'],
        batch_size=args_dic['batch_size'],
